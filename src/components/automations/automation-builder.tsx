...
function blankConfig(type: AutomationStepType): Record<string, unknown> {
  switch (type) {
    ...
    case "create_deal":
      return { pipeline_id: "", stage_id: "", title: "", items: [] }
...
function AutomationDealItemsFields({
  items,
  onChange,
  t,
}: {
  ...
  return (
    <FieldBlock label={t("config.productsLabel")}>
        ...
        {items.map((item, index) => {
          ...
          return (
            <div
              ...
              className="border-border bg-muted/40 grid gap-2 rounded-md border p-2 sm:grid-cols-[minmax(0,1fr)_80px_110px_36px]"
            >
              <select
                aria-label={t("config.productLabel")}
              ...
              >
                ...
                {products.map((product) => (
                  <option
                    key={product.id}
                    value={product.id}
                    disabled={usedProductIds.has(product.id)}
                  >
                    {product.name}
                  </option>
                ))}
              ...
              <Input
                aria-label={t("config.quantityLabel")}
                type="number"
                min="0.001"
                step="0.001"
                value={item.quantity}
              ...
              />
              <Input
                aria-label={t("config.unitPriceLabel")}
                type="number"
                min="0"
                step="0.01"
              ...
              <Button
                ...
                aria-label={t("config.removeProduct")}
